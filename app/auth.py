...
def token_required(f):
    """🔒 Simple token validation (token = Firestore user ID)"""
    @wraps(f)
    def decorated(*args, **kwargs):
        ...
        try:
            uid = token  # token is just the user_doc_id
            ...
            if (
                user_data.get('last_session')
                and user_data.get('last_session') != session_fingerprint
            ):
...
def sanitize_input(data):
    ...
    if isinstance(data, str):
        ...
        data = re.sub(
            r'(?:--|\/\*|\*\/|\b(ALTER|CREATE|DELETE|DROP|INSERT|SELECT|UPDATE|UNION|EXEC)\b)',
            '',
            data,
            flags=re.IGNORECASE,
        )
...
def login():
    """🔑 Email/password + Demo login (simple token = user_doc_id)"""
    print('LOGIN HIT')
    data = request.get_json()
    print('LOGIN RAW DATA:', repr(data))
    ...
    print('LOGIN EMAIL/PASS:', email, password)
    ...
    try:
        ...
        # Simple app token (no Firebase ID/custom token)
        simple_token = user_doc_id

        # Save token in server-side session so /dashboard can read it
        session['token'] = simple_token
        ...
        print('LOGIN SUCCESS, UID:', user_doc_id)
        return jsonify({
            'token': simple_token,
            'user': {
                'uid': user_doc_id,
